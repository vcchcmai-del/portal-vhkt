...
function AdminList({ title, icon: Icon, endpoint, type }) {
  ...
  return <div className="flex flex-col gap-4"><div className="card flex justify-between items-center" style={{ flexWrap: "wrap", gap: 10 }}><div><b>{title}</b><p className="muted" style={{fontSize:12,marginTop:3}}>Theo dõi và cập nhật dữ liệu vận hành.</p></div><div className="flex gap-2"><button className="btn btn-sm" onClick={load}><RefreshCw size={14}/>Tải lại</button><button className="btn btn-red btn-sm" onClick={() => setForm(blank)}><Plus size={14}/>Thêm</button></div></div>
    ...
    <Card pad={false}><div style={{overflowX:"auto"}}><table className="tbl"><thead><tr><th>Mã / ngày</th><th>Nội dung</th><th>Phụ trách</th><th>Trạng thái</th><th style={{textAlign:"right"}}>Thao tác</th></tr></thead><tbody>{filteredRows.map(x=><tr key={x.id}><td className="mono">{x.code || day(x.shift_date || x.duty_date)}</td><td><b>{x.title || x.summary || DUTY[x.duty_type]}</b><br/><span className="muted">{x.site || x.location || x.person_name}</span></td><td>{x.assignee || x.outgoing || x.person_name}</td><td>{STATUS[x.status] || x.shift_name}</td><td style={{textAlign:"right",whiteSpace:"nowrap"}}><ThaoTac xem={{tieuDe: x.title || x.summary || x.code || "Chi tiết", duLieu: x}} onEdit={()=>setForm({...x, due_at:x.due_at ? String(x.due_at).slice(0,16):""})} onDelete={()=>del(x)}/></td></tr>)}</tbody></table>{!filteredRows.length&&<Empty title="Chưa có dữ liệu." hint={rows.length ? "Không có dòng nào khớp bộ lọc." : undefined}/>}</div></Card></div>;
}
