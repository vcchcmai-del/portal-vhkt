...
# (mã đầu việc, tên hiển thị) — đúng 10 đầu việc mảng kỹ thuật.
CATEGORIES = [
...
def _validate(data: TechTaskIn):
    if data.category is not None and data.category not in CATEGORY_IDS:
...
def list_categories(_=Depends(require_module("tech_tasks", "view"))):
    return [{"id": c, "label": l, "hint": CATEGORY_HINTS.get(c, "")} for c, l in CATEGORIES]
...
def admin_tasks_summary(db: Session = Depends(get_db), _=Depends(require_module("tech_tasks", "view"))):
    ...
    return [
        {"category": c, "label": l, **by_category.get(c, {"total": 0, "todo": 0, "doing": 0, "done": 0, "overdue": 0})}
...
def export_tasks_csv(db: Session = Depends(get_db), _=Depends(require_module("tech_tasks", "view"))):
    ...
    for r in rows:
        vals = [
            CATEGORY_LABELS.get(r.category, r.category), r.title or "", r.description or "",
...
PROGRESS_ITEM_LABELS = {code: label for items in PROGRESS_ITEMS.values() for code, label in items}
PROGRESS_ITEM_CATEGORY = {code: cat for cat, items in PROGRESS_ITEMS.items() for code, _ in items}


def _out_progress(row):
    plan, done = row.plan_qty or 0, row.done_qty or 0
    return {
        "id": row.id, "category": row.category,
        "category_label": CATEGORY_LABELS.get(row.category, row.category),
        "item": row.item, "item_label": PROGRESS_ITEM_LABELS.get(row.item, row.item),
...
def _validate_progress(data: ProgressIn):
    if data.category is not None and data.category not in PROGRESS_ITEMS:
        raise HTTPException(400, "Đầu việc không hợp lệ hoặc chưa có hạng mục định lượng.")
    if data.item is not None and data.item not in PROGRESS_ITEM_LABELS:
        raise HTTPException(400, "Hạng mục không hợp lệ.")
    if data.item is not None and data.category is not None and PROGRESS_ITEM_CATEGORY.get(data.item) != data.category:
...
def list_progress_items(category: Optional[str] = None, _=Depends(require_module("tech_tasks", "view"))):
    if category:
        return [{"id": c, "label": l} for c, l in PROGRESS_ITEMS.get(category, [])]
    return [
        {"category": cat, "category_label": CATEGORY_LABELS.get(cat, cat),
         "items": [{"id": c, "label": l} for c, l in items]}
        for cat, items in PROGRESS_ITEMS.items()
    ]
...
def progress_summary(period: Optional[str] = None, db: Session = Depends(get_db),
    ...
    for cat, items in PROGRESS_ITEMS.items():
        for code, label in items:
            b = agg.get((cat, code), {"plan": 0.0, "done": 0.0})
            plan, done = b["plan"], b["done"]
            out.append({
                "category": cat, "category_label": CATEGORY_LABELS.get(cat, cat),
                "item": code, "item_label": label,
                "plan_qty": plan, "done_qty": done, "remaining": plan - done,
                "rate": (done / plan) if plan else None,
            })
    return out
...
def list_progress_centers(category: str, item: str, period: str, db: Session = Depends(get_db),
    ...
    return [_out_progress(x) for x in rows]
...
def export_progress_csv(period: Optional[str] = None, db: Session = Depends(get_db),
    ...
    for r in rows:
        ...
        vals = [
            CATEGORY_LABELS.get(r.category, r.category), PROGRESS_ITEM_LABELS.get(r.item, r.item),
