...
def global_search(q: str, db: Session = Depends(get_db)):
    ...
    for n in db.query(models.News).filter(models.News.deleted_at.is_(None)).all():
        if matches(q, n.title, n.excerpt, n.category):
            hits.append({"kind": "Tin tức", "label": n.title, "sub": n.category, "view": "news"})

    for i in db.query(models.Idea).all():
        if matches(q, i.title, i.dept, i.benefit):
            hits.append({"kind": "Sáng kiến", "label": i.title, "sub": i.dept or "", "view": "ideas"})
