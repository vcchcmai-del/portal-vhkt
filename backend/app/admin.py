...
def export_metrics_csv(board: Optional[str] = None, nhom: Optional[str] = None,
                       ...
                       _=Depends(require_module("dashboard", "view"))):
    """
    Xuất số liệu Dashboard ra CSV — cột "bang" hiện đúng TÊN hiển thị trên
    Dashboard (ví dụ "Sự cố truyền dẫn (theo tỉnh)") thay vì mã nội bộ khó
    đọc (PAKH), để mở tệp lên là hiểu ngay, không cần tra mã. Vẫn nhập ngược
    lên được bình thường: máy chủ nhận cả mã LẪN đúng tên hiển thị này, xem
    bi._chuan_hoa_bang() — nên tệp xuất ra sửa/lọc rồi nhập ngược lên vẫn ghi
    đè đúng dòng cũ (không tạo bản trùng).
