...
def recruitment_summary(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "staffing": {
            ...
            "total_applications": sum(r.applications_received for r in staffing_rows),
            "centers": [
                {"center": r.center, "oft_gap": r.oft_gap, "ft_gap": r.ft_gap,
                 "total_gap": r.oft_gap + r.ft_gap, "applications_received": r.applications_received,
...
def export_candidates_csv(db: Session = Depends(get_db), _=Depends(require_module("recruitment", "view"))):
    ...
    header = ["Họ và tên", "Vị trí ứng tuyển", "Khu vực mong muốn", "Năm sinh",
              "Trình độ", "Chuyên môn", "Người giới thiệu", "Trạng thái", "Ghi chú", "Ngày nộp"]
    lines = [",".join(header)]
    for r in rows:
        vals = [
            r.full_name or "", r.position or "", r.desired_location or "",
...
def _staffing_out(row):
    return {
        ...
        "applications_received": row.applications_received, "posted_channels": row.posted_channels,
...
def admin_list_staffing(db: Session = Depends(get_db), _=Depends(require_module("recruitment", "view"))):
    ...
    return [_staffing_out(x) for x in rows]
...
def admin_create_staffing(data: StaffingIn, db: Session = Depends(get_db),
    ...
    row = models.CenterStaffing(
        ...
        applications_received=data.applications_received or 0, posted_channels=data.posted_channels or 0,
...
def export_staffing_csv(db: Session = Depends(get_db), _=Depends(require_module("recruitment", "view"))):
    ...
    for r in rows:
        vals = [
            ...
            str(r.applications_received or 0), str(r.posted_channels or 0),
